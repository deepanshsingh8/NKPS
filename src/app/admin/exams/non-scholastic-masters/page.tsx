...
export default function NonScholasticMastersPage() {
  ...
  const fetchSubjects = useCallback(async () => {
    const res = await adminFetch("/api/erp/non-scholastic/subjects");
    if (!res.ok) {
  ...
  const fetchSubSubjects = useCallback(async () => {
    const res = await adminFetch("/api/erp/non-scholastic/sub-subjects");
    if (!res.ok) {
  ...
  const saveSubject = async () => {
    ...
    const res = editingSubject
      ...
      : await adminFetch("/api/erp/non-scholastic/subjects", {
          method: "POST",
  ...
  const saveSubSubject = async () => {
    ...
    const res = editingSubSubject
      ...
      : await adminFetch("/api/erp/non-scholastic/sub-subjects", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
