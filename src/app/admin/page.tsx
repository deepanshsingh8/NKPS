...
// Animates a number from 0 → value with eased timing. Starts at the target
// for users with reduced-motion preference so the effect does no work.
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        // Silent fail — header just shows greeting without name.
      }
    };
  ...
  // Visible stat cards (after permission filter while loaded).
  const visibleCards = statCardConfig.filter(
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-white/60 mt-1.5">
                Here&apos;s what&apos;s happening at NKPS today.
              </p>
            </div>
          ...
          ({ key, label, icon: Icon, iconBg, iconColor, accent, href }, i) => {
            ...
            const cardInner = (
              <>
                ...
                {/* Hover CTA: arrow slides in from the right */}
                {!loading && (
